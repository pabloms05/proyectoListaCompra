...
class ListaController extends Controller
{
    ...
    {
        $user = Auth::user();
        $listas = $user->sharedLists()->with('items')->get();
    ...
    public function show(Lista $lista)
    {
        ...
        if (! $lista->userHasAccess($user->id)) {
            // Si el usuario viene desde listas compartidas, redirigir allí en lugar de error
            if (request()->get('from') === 'compartidas') {
                return redirect()->route('listas-compartidas')
                    ->with('error', 'Ya no tienes acceso a esta lista compartida.');
            }
            abort(403, "No tienes permiso para ver esta lista.");
        }

        // Cargar relaciones de productos con su categoría e items para calcular totales
        $lista->load(['productos.categoria', 'items']);
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'nuevos_productos.*.precio' => 'nullable|numeric|min:0',
        ]);
        ...
        if ($request->has('nuevos_productos')) {
            foreach ($request->nuevos_productos as $nuevoProducto) {
                // Buscar si ya existe un producto con mismo nombre y categoría
                $productoExistente = \App\Models\Producto::where('name', $nuevoProducto['nombre'])
                    ->where('categoria_id', $nuevoProducto['categoria_id'])
                    ->first();
                ...
                if ($productoExistente) {
                    ...
                    $producto = \App\Models\Producto::create([
                        ...
                        'created_by_user_id' => Auth::id(), // Marcar como creado por usuario
                    ]);
                }
                ...
                $nuevosProductosCreados[$producto->id_producto] = [
                    ...
                    'precio_unitario' => $nuevoProducto['precio'] ?? $producto->precio ?? 0
                ];
            }
        }
        ...
        if ($request->has('productos')) {
            foreach ($request->productos as $productData) {
                $producto = \App\Models\Producto::find($productData['producto_id']);
    ...
    public function edit(Lista $lista)
    {
        ...
        if (!$isOwner) {
            ...
            // Si no está compartida con él o es lector, redirigir a compartidas
            if (!$sharedUser || $sharedUser->pivot->role === 'lector') {
                return redirect()->route('listas-compartidas')
                    ->with('error', 'Ya no tienes permiso para editar esta lista.');
            }
        }
        ...
        // 2. Obtener los productos actualmente vinculados a esta lista, incluyendo la cantidad y precio
        // Usamos withPivot para obtener los datos de la tabla intermedia
        $productosLista = $lista->productos()->withPivot('cantidad', 'precio_unitario')->get();
    ...
    public function update(Request $request, Lista $lista)
    {
        ...
        if (!$isOwner) {
            ...
            if (!$sharedUser || $sharedUser->pivot->role === 'lector') {
                return redirect()->route('listas-compartidas')
                    ->with('error', 'Ya no tienes permiso para actualizar esta lista.');
            }
        }
        ...
        if ($request->has('productos')) {
            foreach ($request->productos as $productData) {
                ...
                $cantidad = $productData['cantidad'];
                $nuevoPrecio = $productData['precio'] ?? null;
                $producto = \App\Models\Producto::find($productoId);

                // Si el producto fue creado por un usuario Y se proporciona un nuevo precio
                if ($producto && $producto->created_by_user_id && $nuevoPrecio !== null && $nuevoPrecio != $producto->precio) {
                ...
                $syncData[$productoId] = [
                    'cantidad' => $cantidad,
                    'precio_unitario' => $nuevoPrecio ?? ($producto ? $producto->precio : 0)
                ];
            }
        }
    ...
    public function alternarComprado(Request $request, Lista $lista)
    {
        ...
        if (!$lista->userHasAccess($usuario->id)) {
            return redirect()->route('listas-compartidas')
                ->with('error', 'Ya no tienes acceso a esta lista.');
        }
        ...
        if (!$isOwner) {
            ...
            if (!$sharedUser || $sharedUser->pivot->role === 'lector') {
                return redirect()->route('listas-compartidas')
                    ->with('error', 'No tienes permisos de edición en esta lista.');
            }
        }
