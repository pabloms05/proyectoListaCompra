...
<div class="container">
    ...
    <div class="mb-3">
        ...
        <form action="{{ route('listas.destroy', $lista) }}" method="POST" style="display:inline">
            @csrf
            @method('DELETE')
            <button class="btn btn-danger" onclick="return confirm('¿Borrar esta lista?')">Eliminar lista</button>
        </form>
    </div>
    @endif
    ...
    <h3>Productos (todas las categorías)</h3>
    <div class="row">
        @foreach ($lista->categorias as $categoria)
    ...
    <a href="{{ route('categorias.productos.create', [$lista->categorias->first() ?? $lista, /*categoria*/]) }}" class="btn btn-primary">Añadir producto</a>
    ...
    <div class="mt-4">
        ...
        <form action="{{ route('listas.share', $lista) }}" method="POST">
            @csrf
            <div class="mb-2">
                <input type="email" name="user_email" class="form-control" placeholder="Correo del usuario con quien compartir" required>
            </div>
